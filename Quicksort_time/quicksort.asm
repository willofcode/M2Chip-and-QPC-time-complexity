...
array:  .word  12,8,10,5,7,4,2,1	# Define a variable named array as a word (integer) array.
space:  .asciiz  " "
newline: .asciiz "\n"
     	.text
...
part: 			#partition for divide and conquer 
	...
	QS_loop: 
		...
		bne $t6, $zero, QS_for	#if t6=1 then branch to endfor
		sll $t1, $t4, 2		#t1 = j*4
		add $t1, $t1, $a0	#t1 = arr + 4j
		lw $t7, 0($t1)		#t7 = arr[j]
		slt $t8, $t2, $t7	#t8 = 1 if pivot < arr[j], 0 if arr[j]<=pivot
		bne $t8, $zero, QS_if	#if t8=1 then branch to endfif
		addi $t3, $t3, 1	#i=i+1
		move $a1, $t3		#a1 = i
		move $a2, $t4		#a2 = j
	 ...
	 QS_if:
		addi $t4, $t4, 1	#j++
		j QS_loop		#junp back to forloop
